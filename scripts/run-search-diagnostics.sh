...
# from it, run etl/run_search_diagnostics.py as a one-off, READ-ONLY
# Fargate task on the existing research-archive-platform-dev-loader ECS
# task family, wait for it to finish, stream its CloudWatch logs, and
# exit with the container's own exit code.
#
# Mirrors scripts/run-subaward-loader.sh's orchestration exactly
# (build -> register -> run-task -> poll -> tail logs -> exit code),
# scoped to the search-diagnostics script's --suite/--sql-file flags.
# Deliberately reuses the loader family's own VPC/subnet/security-
# group/Secrets-Manager wiring - no bastion, no public RDS, no new
# long-lived infrastructure, per the Global Search Performance Sprint's
# explicit scope.
#
...
#   scripts/run-search-diagnostics.sh --sql-file path/to/check.sql
#   scripts/run-search-diagnostics.sh --suite global-search-baseline --image-uri URI
...
if [[ -z "$SUITE" && -z "$SQL_FILE" ]]; then
  echo "ERROR: one of --suite <name> or --sql-file <path> is required" >&2
  exit 1
fi
if [[ -n "$SUITE" && -n "$SQL_FILE" ]]; then
  echo "ERROR: --suite and --sql-file cannot be combined" >&2
  exit 1
fi
...
if [[ -n "$IMAGE_URI_OVERRIDE" ]]; then
  ...
  echo "=== Building loader image (includes run_search_diagnostics.py) ==="
...
if [[ -n "$SUITE" ]]; then
  COMMAND_ARGS=(python3 run_search_diagnostics.py --ecs --suite "$SUITE")
else
  COMMAND_ARGS=(python3 run_search_diagnostics.py --ecs --sql-file "$SQL_FILE")
fi
