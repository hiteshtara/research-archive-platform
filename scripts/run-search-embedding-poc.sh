...
# run either etl/build_search_embedding_poc.py (--populate) or
# etl/query_search_embedding_poc.py (--query-experiment) as a one-off
# Fargate task on the research-archive-platform-dev-loader task family,
# wait for it to finish, stream its CloudWatch logs, and exit with the
# container's own exit code.
#
# Mirrors scripts/run-search-diagnostics.sh's orchestration exactly.
# Global Search pgvector proof-of-concept only - never touches
# production Global Search. Requires the loader task role's
# bedrock:InvokeModel grant (terraform/modules/ecs/main.tf's
# task_bedrock policy).
#
...
#
# Usage:
#   scripts/run-search-embedding-poc.sh --populate --limit-per-domain 200
#   scripts/run-search-embedding-poc.sh --populate --limit-per-domain 5 --dry-run
#   scripts/run-search-embedding-poc.sh --query-experiment
#   scripts/run-search-embedding-poc.sh --query-experiment --query "diabetes research involving children"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --query-experiment) MODE="query-experiment"; shift ;;
...
if [[ -z "$MODE" ]]; then
  echo "ERROR: one of --populate or --query-experiment is required" >&2
  exit 1
fi
...
if [[ -n "$IMAGE_URI_OVERRIDE" ]]; then
  ...
  echo "=== Building loader image (includes the search-embedding-poc scripts) ==="
...
if [[ "$MODE" == "populate" ]]; then
  COMMAND_ARGS=(python3 build_search_embedding_poc.py --ecs --limit-per-domain "$LIMIT_PER_DOMAIN")
  [[ "$DRY_RUN" == true ]] && COMMAND_ARGS+=(--dry-run)
else
  COMMAND_ARGS=(python3 query_search_embedding_poc.py --ecs)
  for q in "${QUERIES[@]:-}"; do
