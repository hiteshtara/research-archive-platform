...
# and run etl/build_evidence_embedding.py as a one-off Fargate task on
# the research-archive-platform-<env>-loader task family, wait for it to
# finish, stream its CloudWatch logs, and exit with the container's own
# exit code.
#
# Mirrors scripts/run-search-embedding.sh's orchestration exactly - same
# task family, same image, same wait/log/exit-code handling. The only
# difference is which script runs inside the container and its
# arguments: build_evidence_embedding.py is always scoped to exactly one
# Award number and an explicit set of approved evidence document types
# (never production-wide), per
...
#   scripts/run-evidence-embedding.sh --award-number 204713-00133 \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --award-number) AWARD_NUMBER="$2"; shift 2 ;;
    --document-types) DOCUMENT_TYPES="$2"; shift 2 ;;
...
if [[ -z "$AWARD_NUMBER" ]]; then
  echo "ERROR: --award-number is required - this script never processes production-wide" >&2
  exit 1
fi
...
if [[ -n "$IMAGE_URI_OVERRIDE" ]]; then
  ...
  echo "=== Building loader image (includes build_evidence_embedding.py) ==="
...
COMMAND_ARGS=(python3 build_evidence_embedding.py --ecs --award-number "$AWARD_NUMBER")
[[ -n "$DOCUMENT_TYPES" ]] && COMMAND_ARGS+=(--document-types "$DOCUMENT_TYPES")
