CREATE TABLE IF NOT EXISTS archive.research_record (
    ...
    CONSTRAINT ux_research_record_source
        UNIQUE (record_type, source_system, source_identifier)
);

CREATE INDEX IF NOT EXISTS ix_research_record_business_identifier
    ON archive.research_record (business_identifier);

CREATE INDEX IF NOT EXISTS ix_research_record_type_status
    ON archive.research_record (record_type, status_description);

CREATE INDEX IF NOT EXISTS ix_research_record_lead_person
    ON archive.research_record (lead_person_buid);

CREATE INDEX IF NOT EXISTS ix_research_record_unit
    ON archive.research_record (responsible_unit);

CREATE INDEX IF NOT EXISTS ix_research_record_title_trgm
    ON archive.research_record
    USING GIN (title gin_trgm_ops);

CREATE INDEX IF NOT EXISTS ix_research_record_search
    ON archive.research_record
    USING GIN (
        to_tsvector(
            'english',
            ...
            COALESCE(lead_person_name, '') || ' ' ||
            COALESCE(business_identifier, '')
        )
    );
